...
void GenerateTypes(string modelPath, string header, IReadOnlyCollection<string> traversedHeaders, string installRoot, string outputRoot, string intermediateRoot) {
    ...
    var response = CreateCommonArguments(installRoot);
    response.Add("--namespace=" + bindingsNamespace);
    response.Add("--with-access-specifier=*=Public");
    response.Add("--output=" + Quote(outputPath));
    response.Add("--file=" + Quote(header));
    foreach (var traversedHeader in traversedHeaders) {
        response.Add("--traverse=" + Quote(traversedHeader));
...
void GenerateOpaqueTypes(IEnumerable<string> names, string outputRoot) {
    ...
    foreach (var name in names.Distinct(StringComparer.Ordinal).Order(StringComparer.Ordinal)) {
        lines.Add(name switch {
            "sockaddr_in" => "[StructLayout(LayoutKind.Sequential)] public unsafe struct sockaddr_in { public short sin_family; public ushort sin_port; public fixed byte sin_addr[4]; public fixed byte sin_zero[8]; }",
            "sockaddr_in6" => "[StructLayout(LayoutKind.Sequential)] public unsafe struct sockaddr_in6 { public ushort sin6_family; public ushort sin6_port; public uint sin6_flowinfo; public fixed byte sin6_addr[16]; public uint sin6_scope_id; }",
            "_SYSTEMTIME" => "[StructLayout(LayoutKind.Sequential)] public unsafe struct _SYSTEMTIME { public fixed ushort values[8]; }",
            "_FILETIME" => "[StructLayout(LayoutKind.Sequential)] public struct _FILETIME { public uint dwLowDateTime; public uint dwHighDateTime; }",
            _ => $"public struct {name} {{ }}"
        });
    }
